...
#define bc_log( severity, format, args... )  {                                              \
    ...
    if ( bc_log_levels  & LOG_MASK( severity ) ) {                                          \
        ...
        if ( bc_log_channels & BC_LOG_STDERR )                                              \
            fprintf( stderr, "%s::%s:%d " format, __FILE__, __func__, __LINE__, ##args );   \
    }                                                                                       \
    if ( bc_log_channels & BC_LOG_SYSLOG ) {                                                \
        openlog("bootconfig", LOG_PID, LOG_USER);                                           \
        syslog( severity, "%s::%s:%d " format, __FILE__, __func__, __LINE__, ##args );      \
        closelog();                                                                         \
    }                                                                                       \
}
